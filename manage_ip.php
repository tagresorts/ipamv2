...
// ------------------------------------------------------------------
// Process Actions: add, edit, delete, list
// ------------------------------------------------------------------
...
if ($action === 'add' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        header("Location: manage_ip.php?action=list");
        exit;
...
// ----------------------
if ($action === 'list') {
    // For admin, show all IPs; for non‑admins, only show IPs from their assigned companies.
    if ($_SESSION['role'] === 'admin') {
    ...
    $sql = "SELECT
            ...
            WHERE ips.company_id IN ($placeholders)
    ...
    <?php elseif ($action === 'edit'): ?>
      ...
      <form method="POST" class="form-container">
        ...
        <div class="full-width">
          ...
          <a href="manage_ip.php?action=list" class="btn">Cancel</a>
        </div>
      </form>
    <?php endif; ?>
  </div>
